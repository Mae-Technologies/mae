...
--   ]
-- }
CREATE OR REPLACE FUNCTION app.create_table_from_spec(p_spec jsonb)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = app
...
END IF;
  ...
  BEGIN
    ...
    PERFORM app.upsert_table_column_policy(v_table_name, immutable_cols);
  ...
  ---------------------------------------------------------------------------
  PERFORM app.apply_table_acl(v_table_name, insertable_extras, updatable_extras);
