...
impl TestContext {
    ...
    pub async fn scoped_connection(&self,) -> Result<PgConnection,> {
        ...
        let mut conn =
            PgConnection::connect(&url,).await.context("failed to connect to Postgres",)?;
        conn.execute(format!(r#"SET search_path TO "{}", public"#, schema).as_str(),).await?;
        Ok(conn,)
    }
}
...
async fn postgres_host_port() -> Result<(String, u16,),> {
    ...
    let host = c.get_host().await.context("failed to get host",)?.to_string();
    let port = c.get_host_port_ipv4(5432,).await.context("failed to get port",)?;
    ...
    let pool_ref = POOL
        .get_or_init(|| async {
            ...
            let init: Result<PgPool,> = async {
                ...
                sqlx::migrate!("./migrations").run(&pool,).await.context("migrations failed",)?;
